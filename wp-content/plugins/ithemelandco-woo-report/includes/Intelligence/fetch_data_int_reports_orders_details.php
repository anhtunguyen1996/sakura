<?php
	...
	foreach($order_items as $key => $order_item  ){
		...
		if($order_shipping){
			$order_shipping_v=($order_shipping) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($order_shipping);
			$order_shipping_html='<tr><td class="pw-left-align pw-black"> '.esc_html__("Shipping",__IT_REPORT_WCREPORT_TEXTDOMAIN__).' </td><td class="pw-right-align pw-black"> '.$order_shipping_v.'</td></tr>';
		}
	}

	/////TAX/////
	$tax_name=$it_rpt_main_class->it_oin_list($order_id,'tax');
	...
	foreach( $order->get_items( 'tax' ) as $item_id => $item_tax ){
		...
		$tax_name_rate_html.='<tr><td class="pw-left-align pw-black "> '.$item_tax_label.' </td><td class="pw-right-align pw-black"> '.$it_rpt_main_class->price(($item_tax_total+$item_tax_shipping_total)).'</td></tr>';
	}


	//$net_amnt=($net_amnt) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($net_amnt);
	$gross_amnt+=$tax_total+$order_shipping;
	$gross_amnt=($net_amnt) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($net_amnt);
	$net_amnt=($net_amnt) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($net_amnt);
	...
	foreach($related_array as $related){
		...
		$it_table_value=($order->get_total()) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($order->get_total());
	...
	//////////////////////////////////////////////////////
	$it_table_value_status = isset($current_order_status) ? $current_order_status : '';

	if($it_table_value_status=='wc-completed')
		$it_table_value_status = '<span class="awr-order-status awr-order-status-'.sanitize_title($it_table_value_status).'" >'.ucwords(esc_html__($it_table_value_status, __IT_REPORT_WCREPORT_TEXTDOMAIN__)).'</span>';
	...
	$order_statuses = array(
		'wc-pending'    => esc_html__( 'Pending payment', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-processing' => esc_html__( 'Processing', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-on-hold'    => esc_html__( 'On hold', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-completed'  => esc_html__( 'Completed', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-cancelled'  => esc_html__( 'Cancelled', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-refunded'   => esc_html__( 'Refunded', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
		'wc-failed'     => esc_html__( 'Failed', 'Order status', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ),
	);
	$order_statuses_html='';
	...
	$current_order_status= str_replace("Wc-","",$it_table_value_status);
	echo '
		<div class="int-awr-box pw-pr-cus-detail">
            ...
            <div class="pw-cus-detail-header">
                ...
                <div class="col-xs-6 col-sm-6 col-md-3 pw-align-right">
                    ...
                    '.$items_count.' '.esc_html__( 'item(s)', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ).'
                    </div>
                </div>
                <div class="col-xs-6 col-sm-6 col-md-3">

                    <div data-order-id="" class="pw-xs-font pw-backed-lbl pw-green-back pull-right order-lbl ">'.$current_order_status.' | <span class="it_intelligence_change_order_status"> '.esc_html__( 'Change', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ).'</span></div>
                    <div class="clear-fx"></div>
                    <select class="pull-right  pw-xs-font order-select it_intelligence_order_status">
						'.$order_statuses_html.'
					</select>
                ...
                <div class="pw-box-padder">
                    <div class="col-xs-12 col-md-8">
                        <table class="pw-simple-dashed-tbl pw-sm-font">
                                ...
                                <tr>
                                    <td class="pw-left-align pw-black pw-md-font">
                                        '.esc_html__('Total',__IT_REPORT_WCREPORT_TEXTDOMAIN__).': '.$items_count.' '.esc_html__('items',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
                                    </td>
                        ...
                        <div class="pw-note-cnt">
                            ...
                            <textarea cols="3" class="pw-sm-font it_intelligence_note_text" placeholder="'.esc_html__('click here to add note for this order...',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'"></textarea>
                            <span class="pw-green it_intelligence_note_resp pw-md-font"></span>
                            <button class="pw-button pw-pull-right it_intelligence_note_text_save"  data-id="'.$order_id.'" data-target="order">'.esc_html__('Save',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</button>
                            <div class="clear-fx"></div>
                        </div>
                        <div class="pw-prelated-cnt">
                            <div class="pw-val pwl-lbl">'.esc_html__('RELATED TRANSACTIONS',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                    ...
                    <div class="col-xs-12 col-md-4 pw-info-cnt">
                        ...
                        <div class="pw-info">
                            <div class="pw-val pwl-lbl">'.esc_html__('DATE AND TIME',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                            <div class="pw-black pw-sm-font">'.$order_date.'</div>
                        </div>

                        <div class="pw-info">
                            <div class="pw-val pwl-lbl">'.esc_html__('PAYMENT METHOD',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                            <div class="pw-black pw-sm-font">'.$payment_method.'</div>
                        </div>

                        <div class="pw-info">
                            <div class="pw-val pwl-lbl">'.esc_html__('BILLING ADDRESS',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                            <div class="pw-black pw-sm-font">'.$billing_info.'</div>
                        </div>

                        <div class="pw-info">
                            <div class="pw-val pwl-lbl">'.esc_html__('SHIPPING ADDRESS',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
