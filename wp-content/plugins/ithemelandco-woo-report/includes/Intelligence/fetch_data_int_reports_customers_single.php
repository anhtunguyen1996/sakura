<?php
    ...
    $sql="SELECT it_posts.post_status,postmeta.meta_value AS billing_email, billing_country.meta_value as billing_country, DATE_FORMAT(it_posts.post_date,'%m/%d/%Y') AS order_date, it_woocommerce_order_items.order_id AS order_id,	it_woocommerce_order_items.order_item_name AS product_name,	it_woocommerce_order_items.order_item_id	AS order_item_id, woocommerce_order_itemmeta.meta_value AS woocommerce_order_itemmeta_meta_value, (it_woocommerce_order_itemmeta2.meta_value/it_woocommerce_order_itemmeta3.meta_value) AS sold_rate, (it_woocommerce_order_itemmeta4.meta_value/it_woocommerce_order_itemmeta3.meta_value) AS product_rate, (it_woocommerce_order_itemmeta4.meta_value) AS item_amount, (it_woocommerce_order_itemmeta2.meta_value) AS item_net_amount, (it_woocommerce_order_itemmeta4.meta_value - it_woocommerce_order_itemmeta2.meta_value) AS item_discount,	it_woocommerce_order_itemmeta2.meta_value AS total_price, count(it_woocommerce_order_items.order_item_id) AS product_quentity, woocommerce_order_itemmeta.meta_value AS product_id ,woocommerce_order_itemmeta_v.meta_value AS variation_id ,it_woocommerce_order_itemmeta3.meta_value AS 'product_quantity'	,it_posts.post_status AS post_status ,it_posts.post_status AS order_status FROM {$wpdb->prefix}woocommerce_order_items as it_woocommerce_order_items LEFT JOIN {$wpdb->prefix}posts as it_posts ON it_posts.ID=it_woocommerce_order_items.order_id	LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as woocommerce_order_itemmeta ON woocommerce_order_itemmeta.order_item_id	=	it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as woocommerce_order_itemmeta_v ON woocommerce_order_itemmeta_v.order_item_id	=	it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as it_woocommerce_order_itemmeta2 ON it_woocommerce_order_itemmeta2.order_item_id	=	it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as it_woocommerce_order_itemmeta3 ON it_woocommerce_order_itemmeta3.order_item_id	= it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as it_woocommerce_order_itemmeta4 ON it_woocommerce_order_itemmeta4.order_item_id	=	it_woocommerce_order_items.order_item_id AND it_woocommerce_order_itemmeta4.meta_key='_line_subtotal' LEFT JOIN {$wpdb->prefix}postmeta as billing_country ON billing_country.post_id = it_posts.ID LEFT JOIN {$wpdb->prefix}postmeta as postmeta ON postmeta.post_id=it_woocommerce_order_items.order_id Where it_posts.post_type = 'shop_order' AND it_posts.ID<>'$order_id' AND billing_country.meta_key	= '_billing_country' AND postmeta.meta_key='_billing_email' AND woocommerce_order_itemmeta.meta_key = '_product_id' AND woocommerce_order_itemmeta_v.meta_key = '_variation_id' AND it_woocommerce_order_itemmeta2.meta_key='_line_total' AND it_woocommerce_order_itemmeta3.meta_key='_qty' AND postmeta.meta_value IN ('$customer_email') AND it_posts.post_status IN ($it_order_status) AND it_posts.post_status NOT IN ('trash') GROUP BY order_item_id ORDER BY order_date DESC";
    $order_items=$wpdb->get_results($sql);
    //print_r($order_items);
    //echo $sql;

	$back_html=esc_html__("Back to product",__IT_REPORT_WCREPORT_TEXTDOMAIN__);
	if($page_id=='it_rpt_fetch_single_customer_main'){
		$back_html=esc_html__("Back to all customers",__IT_REPORT_WCREPORT_TEXTDOMAIN__);
	}
...
//echo $order_item -> order_id;
        $order_refund_amnt= $it_rpt_main_class->it_get_por_amount($order_item -> order_id);
        ...
        //Order Total
        $it_table_value = isset($order_item -> order_total) ? ($order_item -> order_total)-$part_refund : 0;
        $it_table_value = $it_table_value == 0 ? 0 : $it_table_value;
        ...
        if($new_order){
            ...
            $customer_array[$order_item->order_id]['total']=$it_table_value;

	        $activity_array[$order_item->order_id][$p_id]['total']=$it_table_value;
        ...
        $items_count = $it_rpt_main_class->it_get_oi_count($order_item->order_id,'line_item');
        ...
        //GROSS AMOUNT
        $order_refund_amnt= $it_rpt_main_class->it_get_part_order_refund_amount($order_item -> order_id);
    ...
    $net_amnt=($net_amnt) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($net_amnt);
    $gross_amnt=($gross_amnt) == 0 ? $it_rpt_main_class->price(0) : $it_rpt_main_class->price($gross_amnt);
    ...
    foreach ($activity_array as $order_ids => $activity){
        ...
        foreach($activity_array[$order_ids] as $prod_id => $act_products){
	        ...
	        $year_text[0]=($diff->y>1 ? $diff->y. " ".esc_html__('years',__IT_REPORT_WCREPORT_TEXTDOMAIN__):($diff->y==1 ? $diff->y. " ".esc_html__('year',__IT_REPORT_WCREPORT_TEXTDOMAIN__):""));
	        $year_text[1]=($diff->m>1 ? $diff->m. " ".esc_html__('months',__IT_REPORT_WCREPORT_TEXTDOMAIN__):($diff->m==1 ? $diff->m. " ".esc_html__('month',__IT_REPORT_WCREPORT_TEXTDOMAIN__):""));
	        $year_text[2]=($diff->d>1 ? $diff->d. " ".esc_html__('days',__IT_REPORT_WCREPORT_TEXTDOMAIN__):($diff->d==1 ? $diff->d. " ".esc_html__('day',__IT_REPORT_WCREPORT_TEXTDOMAIN__):""));


	        $diff_days= implode(array_filter($year_text),', ')." ".esc_html__('ago',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
	        if(implode(array_filter($year_text),', ')=='')
	        	$diff_days=esc_html__("Today",__IT_REPORT_WCREPORT_TEXTDOMAIN__);

	        $activity_products_name.='<div class="pw-xs-font">'.$act_products['quantity'].' x '.$act_products['name'].'</div>';
	        $activity_products_item_amount.='<div class="pw-xs-font">'.$it_rpt_main_class->price($act_products['item_amount']).'</div>';
	        $activity_products_date='<span class="pw-xs-font pw-val">'.$act_products['date'].' - '.$diff_days.'</span>';
	        $activity_products_total='<div class="pw-sm-font">'.$it_rpt_main_class->price($act_products['item_amount']).'</div>';
        }
	    $activity_html.='
            <tr>
                <td class="pw-left-align ">
                    <div>
                        <span class="pw-sm-font ">
                            '.esc_html__('Purchase',__IT_REPORT_WCREPORT_TEXTDOMAIN__).' <i class="fa fa-check pw-green"></i>
                        </span>
                        '.$activity_products_date.'
                    </div>
                    '.$activity_products_name.'
    ...
    foreach($products_array as $key => $product){
        ...
        <tr>
            ...
            <td class="pw-right-align pw-md-font pw-green">
                '.$it_rpt_main_class->price($product['item_amount']).'
            </td>
        </tr>';
    }
    $product_html.='
    <tr>
        <td class="pw-left-align pw-black pw-md-font">
            '.esc_html__('Total',__IT_REPORT_WCREPORT_TEXTDOMAIN__).' :
        </td>
        ...
        <td class="pw-right-align pw-black pw-md-font">
            '.$it_rpt_main_class->price($gross_amnt_prd).'
        </td>
    </tr>';

    $part_refund_amnt=$it_rpt_main_class->price($part_refund_amnt);
    $part_refund_amnt=$it_rpt_main_class->price($total_all_refund);
	...
	if ( ! $order_id ) {
		...
		$customer_state=$it_rpt_main_class->it_get_woo_bsn($customer_coutry,$customer_state);
	...
	$registered= esc_html__('Joined',__IT_REPORT_WCREPORT_TEXTDOMAIN__).' '.implode(array_filter($year_text),', ')." ".esc_html__('ago',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
	if(implode(array_filter($year_text),', ')=='')
		$registered= esc_html__('Joined',__IT_REPORT_WCREPORT_TEXTDOMAIN__)." ".esc_html__('Today',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
    ...
    <div class="pw-cols col-xs-12 col-md-12">
    	<span class="it_rpt_fetch_single_customer_back">'.$back_html.' |  </span>
    	<span class="it_rpt_fetch_single_customer_prev"  title="Previous customer">
			<i class="fa fa-angle-left fa-2x"></i>
		</span>
		<span class="it_rpt_fetch_single_customer_next" title="Next customer">
    ...
    <div class="pw-cols col-xs-12 col-md-6">
        <div class="int-awr-box int-fixed-height-box-pr">
            <div class="awr-title">
                <h3>
                    <i class="fa fa-money"></i>'.esc_html__('Products',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
                </h3>
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="int-awr-box pw-center-align pw-pr-sum-box">
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <div class="pw-box-padder">
                            ...
                            <div class="pw-val pwl-lbl">'.esc_html__('REVENUW',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                        </div>
                    </div>
                    ...
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <div class="pw-box-padder">
                            ...
                            <div class="pw-val pwl-lbl">'.esc_html__('ORDERS',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                        </div>
                    </div>
                    ...
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <div class="pw-box-padder">
                            ...
                            <div class="pw-val pwl-lbl">'.esc_html__('ITEMS',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="int-awr-box int-fixed-height-box">
                    <div class="awr-title">
                        <h3>
                            <i class="fa fa-money"></i>'.esc_html__('Activity',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
                        </h3>
