<?php
	...
	$sql = "SELECT it_woocommerce_order_items.order_item_name	AS 'product_name' ,it_woocommerce_order_items.order_item_id	AS order_item_id ,SUM(woocommerce_order_itemmeta.meta_value)	AS 'quantity' ,SUM(it_woocommerce_order_itemmeta6.meta_value)	AS 'total_amount' ,it_woocommerce_order_itemmeta7.meta_value	AS product_id ,it_postmeta_customer_user.meta_value	AS customer_id ,DATE(shop_order.post_date) AS post_date ,it_postmeta_billing_billing_email.meta_value	AS billing_email ,CONCAT(it_postmeta_billing_billing_email.meta_value,' ',it_woocommerce_order_itemmeta7.meta_value,' ',it_postmeta_customer_user.meta_value)	AS group_column ,CONCAT(it_postmeta_billing_first_name.meta_value,' ',postmeta_billing_last_name.meta_value)	AS billing_name	FROM {$wpdb->prefix}woocommerce_order_items as it_woocommerce_order_items	LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as woocommerce_order_itemmeta ON woocommerce_order_itemmeta.order_item_id=it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as it_woocommerce_order_itemmeta6 ON it_woocommerce_order_itemmeta6.order_item_id=it_woocommerce_order_items.order_item_id LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as it_woocommerce_order_itemmeta7 ON it_woocommerce_order_itemmeta7.order_item_id=it_woocommerce_order_items.order_item_id	LEFT JOIN {$wpdb->prefix}posts as shop_order ON shop_order.id=it_woocommerce_order_items.order_id LEFT JOIN {$wpdb->prefix}postmeta as it_postmeta_billing_first_name ON it_postmeta_billing_first_name.post_id	= it_woocommerce_order_items.order_id LEFT JOIN {$wpdb->prefix}postmeta as postmeta_billing_last_name ON postmeta_billing_last_name.post_id	=	it_woocommerce_order_items.order_id LEFT JOIN {$wpdb->prefix}postmeta as it_postmeta_billing_billing_email ON it_postmeta_billing_billing_email.post_id	=	it_woocommerce_order_items.order_id LEFT JOIN {$wpdb->prefix}postmeta as it_postmeta_customer_user ON it_postmeta_customer_user.post_id	=	it_woocommerce_order_items.order_id WHERE woocommerce_order_itemmeta.meta_key	= '_qty' AND it_woocommerce_order_itemmeta6.meta_key	= '_line_total' AND it_woocommerce_order_itemmeta7.meta_key = '_product_id' AND it_woocommerce_order_itemmeta7.meta_key = '_product_id' AND it_postmeta_billing_first_name.meta_key	= '_billing_first_name' AND postmeta_billing_last_name.meta_key	= '_billing_last_name' AND it_postmeta_billing_billing_email.meta_key	= '_billing_email' AND it_postmeta_customer_user.meta_key	= '_customer_user' AND (DATE(shop_order.post_date) BETWEEN '2018-01-01' AND '2018-07-11') AND shop_order.post_status IN ('wc-processing','wc-on-hold','wc-completed') AND shop_order.post_status NOT IN ('{$this->otder_status_hide}') GROUP BY group_column ORDER BY billing_name ASC, product_name ASC, total_amount DESC";
...
$body.='<table width="100%" cellspacing="0">
						<tr>
				            <td style="padding: 10px; background-color: #fac34f; color: #fff; font-size: 13px; text-transform: uppercase; font-weight: bold">
				                '.esc_html__("Product SKU",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 13px; text-transform: uppercase; font-weight: bold; border-right: 1px solid #fff">
				                '.esc_html__("Customer Name",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 14px; text-transform: uppercase; font-weight: bold">
				                '.esc_html__("Customer Email",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 14px; text-transform: uppercase; font-weight: bold">
				                '.esc_html__("Product Name",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 14px; text-transform: uppercase; font-weight: bold">
				                '.esc_html__("Sale Qty.",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 14px; text-transform: uppercase; font-weight: bold">
				                '.esc_html__("Current Stock",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'
				            </td>
				            <td style="padding: 10px; background-color: #4d4d4f; color: #fff; font-size: 14px; text-transform: uppercase; font-weight: bold">
