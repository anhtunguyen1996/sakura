...
if($file_used=="sql_table")
{
	...
	$columns=array(
		...
		array('lable'=>esc_html__('View Chart',__IT_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
	);
...
it_posts.ID=meta.post_id $it_order_status_join where meta.meta_key='it_cartitems' AND it_posts.post_type='carts' $date_condition $it_order_status_condition $it_paid_customer_condition $it_product_id_condition ";
...
elseif($file_used=="data_table"){
	...
	foreach ( $product_array as $key=>$pitem ) {
		...
		$datatable_value .= ( "<td style='" . $display_class . "'>" );
		$datatable_value .= '<div class="it_product_chart_click" data-product-id="'.$key.'" data-product-title="'.$pitem['title'].'"><i class="fa fa-1x fa-bar-chart-o"></i></div>';
		$datatable_value .= ( "</td>" );
