...
class IT_Edit_Interface {
	...
	public function __construct() {
		...
		$this->receipt = new IT_Cart_Receipt();
...
public function cart_status_customer_metabox(){
	...
	elseif($this->receipt->status() == "Converted")
	{
		...
		echo __( '<span style="color:gray";>Name Not Available</span>', 'woocommerce_cart_reports' ) . it_tooltip(esc_html__('No customer name information available for carts created by non-logged-in Guests.', 'woocommerce_cart_reports'), false);
...
public function cart_status_metabox(){
	...
	switch($show_custom_state) {
		...
		case "Abandoned":
			$tooltip = it_tooltip(__("A cart becomes <i>Abandoned</i> when the cart's owner has not accessed the site in an amount of time exceeding your timeout set in the <i>WooCommerce Cart Reports</i> settings page. Your current timeout is set to $timeout_min Minutes. Don't worry! The cart will become open again when the customer returns.", 'woocommerce_cart_reports'), false);
		break;

		case "Open":
			$tooltip = it_tooltip(__("A cart is considered <i>Open</i> when the customer has accessed the site with items in the cart, within the timeout set in the <i>WooCommerce Cart Reports</i> settings page. Your current timeout is set to $timeout_min Minutes.", 'woocommerce_cart_reports'), false);
		break;

		case "Converted":
			$tooltip = it_tooltip(__("A cart becomes <i>Converted</i> when the customer purchases the cart contents. Congrats :) ", 'woocommerce_cart_reports'), false);
		break;
	}
...
<?php
	...
	if($ip != '') {
		...
		echo "<span style='color:lightgray;'>" . esc_html__('Not Available','woocommerce_cart_reports') . it_tooltip(esc_html__('You have probably unchecked "Log IP Address" in the WooCommerce Cart Reports settings panel.', 'woocommerce_cart_reports'), false) . "</span><br />";
	}
...
public function woocommerce_cart_items_meta_box($post) {
	...
	$order_items = (array) maybe_unserialize( get_post_meta($post->ID, 'it_cartitems', true) );
	?>
<div class="woocommerce_cart_reports_items_wrapper">
  ...
  <table cellpadding="0" width="100%" cellspacing="0" class="woocommerce_cart_reports_items">
    ...
    <tbody id="cart_items_list">
      ...
      <?php it_tooltip(esc_html__('When a customer adds
