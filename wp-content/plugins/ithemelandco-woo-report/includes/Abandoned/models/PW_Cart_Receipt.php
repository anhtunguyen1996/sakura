...
class IT_Cart_Receipt
{
    ...
    public function load_receipt($post_id)
    {
        ...
        //$products = get_post_meta($post_id,'it_cartitems',true);

        $this->post_title = $post->post_title;
        $this->post_author = $post->post_author;
        $this->post_date = $post->post_date;

        $this->ip_address = get_post_meta($this->post_id, 'it_ip_address', true);
        //$this->user_agent = get_post_meta($this->post_id,'it_user_agent',true);

        $this->last_updated = get_post_meta($this->post_id, "it_last_updated", true) + ($offset * 3600);

        $this->last_connected = $post->post_date;
        $this->created = get_post_meta($this->post_id, "it_cart_created", true);
...
//print_r($carts);
  //      die("dsds");
    ...
    public function update_receipt()
    {
        ...
        update_post_meta($this->post_id, 'it_cartitems', $this->add_titles_to_cart_items($this->products));
        $post_updated = array(
           'ID' => $this->post_id,
        ...
        update_post_meta($this->post_id, "it_last_updated", time());
    }
    ...
    {
        update_post_meta($this->post_id, "it_last_connected", time());
    }
    ...
    {
        $order_id = get_post_meta($this->post_id, "it_order_id", true);
        if ($order_id != '') {
            return get_post_meta($this->post_id, "it_order_id", true);
        } else {
            return false;
        }
    }
    ...
    public function save_order_id($order_id)
    {
        ...
        update_post_meta($this->post_id, 'it_order_id', $order_id);
    ...
    public function insert_receipt()
    {
        ...
        if (isset($woocommerce_cart_reports_options['logip'])) { //Only save if the user has checked "Log IP" in the setting page
            update_post_meta($this->post_id, "it_ip_address", $this->ip_address);
            update_post_meta($this->post_id, "it_user_agent", $this->user_agent);
        }

        wp_set_object_terms($this->post_id, 'open', "shop_cart_status"); //Saving the open/converted as meta - much faster than postmeta!

        update_post_meta($this->post_id, 'it_cartitems', $this->add_titles_to_cart_items($this->products)); //Save the Cart products from the woocommerce object

        update_post_meta($this->post_id, 'it_cart_created', time()); //Save created time
        update_post_meta($this->post_id, 'it_last_updated', time()); //Initialize the updated time
    }
    ...
    public function print_cart_actions()
    {
        global $post;
        global $user_info;
        $cart_receipt = new IT_Cart_Receipt();
        ...
        switch ($status) {
            ...
            $order_id = '';
            $order_id = get_post_meta($post->ID, 'it_order_id', true);

            if ($order_id > 0 && $order_id != '') {
                $order_post = get_post($order_id);
                $order_link = get_admin_url('', 'post.php?post='.$order_id.'&action=edit');
                $actions[] = new IT_Cart_Action($order_link, 'View Order');
            }
            do_action("it_carts_action_converted", $actions);
        ...
        case 'Open':
            ...
            if ($cart_receipt->email() != false && $cart_receipt->email() != '') {
                ...
                $actions[] = new IT_Cart_Action($email_link, "Email $full_name");
            }

            do_action("it_carts_action_open", $actions);
        ...
        case 'Abandoned':
            ...
            do_action("it_carts_action_abandoned");
