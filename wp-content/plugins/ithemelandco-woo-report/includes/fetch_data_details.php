...
//print_r($ff_columns);
    ...
    if ($it_detail_view == "yes") {
        ...
        $columns = array(
            ...
            array('lable' => esc_html__('Customer', __IT_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
            array('lable' => esc_html__('Email', __IT_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
            array('lable' => esc_html__('Phone No.', __IT_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show'),
            ...
            //CUSTOM WORK - 4187
            //array('lable'=>esc_html__('Transaction ID',__IT_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show'),
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('11899', __CUSTOMWORK_ID__)) {
            ...
            $extra_cols[] = array('lable'  => esc_html__('Delivery Date', __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                                  'status' => 'show'
            );
            $extra_cols[] = array('lable'  => esc_html__('Shipping Date', __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                                  'status' => 'show'
            );
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('12679', __CUSTOMWORK_ID__)) {
            $extra_cols   = array();
            $extra_cols[] = array('lable' => esc_html__('Clinic Type', __IT_REPORT_WCREPORT_TEXTDOMAIN__), 'status' => 'show');
            array_splice($columns, 4, 0, $extra_cols);
        }
        ...
        if ($ff_columns_in) {
            $array_index = 11;
            //$brands_cols[]=array('lable'=>esc_html__(__IT_BRAND_LABEL__,__IT_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
        ...
        if (__IT_BRAND_SLUG__) {
            $brands_cols[] = array('lable'  => __(__IT_BRAND_LABEL__, __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                                   'status' => 'show'
            );
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('14957', __CUSTOMWORK_ID__)) {
            $extra_cols   = array();
            $extra_cols[] = array('lable'  => esc_html__('Transaction ID', __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                                  'status' => 'show'
            );
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('14957', __CUSTOMWORK_ID__)) {
            $extra_cols   = array();
            $extra_cols[] = array('lable'  => esc_html__('Childs Farm Order No', __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                                  'status' => 'show'
            );
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('19445', __CUSTOMWORK_ID__)) {
            ...
            foreach ($easy_checkout_field as $key => $field) {
                ...
                if (strpos($key, 'billing_field') !== false) {
                    ...
                    $tlabel       = $field['label'];
                    $extra_cols[] = array(
                        'lable'  => __($tlabel, __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                        'status' => 'show'
                    );
        ...
        if (is_array(__CUSTOMWORK_ID__) && in_array('14957', __CUSTOMWORK_ID__)) {
            $extra_cols   = array();
            $extra_cols[] = array(
                'lable'  => esc_html__('Childs Farm Order No', __IT_REPORT_WCREPORT_TEXTDOMAIN__),
                'status' => 'show'
            );
            array_splice($columns, 1, 0, $extra_cols);
        }
...
//                    $datatable_value.=("</td>");
    ...
    <form class='alldetails search_form_report' action='' method='post'>
        ...
        <div class="col-md-6">
            ...
            <span class="awr-form-icon"><i class="fa fa-calendar"></i></span>
            <input name="it_id_order" type="text" class="" placeholder="<?php _e('Separate IDs with (,) Example : 1,2',
