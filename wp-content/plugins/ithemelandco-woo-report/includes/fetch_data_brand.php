<?php
	if($file_used=="sql_table")
	{
		...
		$it_parent_brand_id		= $this->it_get_woo_requests('it_parent_brand_id','-1',true);

		$it_child_brand_id	= $this->it_get_woo_requests('child_brand_id','-1',true);
		...
		$it_list_parent_brand			= $this->it_get_woo_requests('list_parent_brand',NULL,false);
		$brand_id			= $this->it_get_woo_requests('it_brand_id','-1',true);
		$it_group_by_parent_brand			= $this->it_get_woo_requests('group_by_parent_brand','-1',true);
		///////////HIDDEN FIELDS////////////
		...
		////ADDED IN VER4.0
		//BRANDS ADDONS
		$it_parent_brand_id=$this->it_get_form_element_permission('it_parent_brand_id',$it_parent_brand_id,$key);
		///////////////////////////
		...
		//PARENT CATEGORY
		$it_parent_brand_id_condition='';

		//CHILD CATEGORY
		$it_child_brand_id_condition='';

		//LIST PARENT CATEGORY
		$it_list_parent_brand_condition='';
		...
		,it_terms_product_id.term_id AS brand_id
		,it_terms_product_id.name AS brand_name
		,it_term_taxonomy_product_id.parent AS parent_brand_id
		,it_terms_parent_product_id.name AS parent_brand_name";
		...
		AND it_term_taxonomy_product_id.taxonomy 						= 'product_brand'
		...
		if($it_parent_brand_id != NULL and $it_parent_brand_id != "-1"){
			$it_parent_brand_id_condition= " AND it_term_taxonomy_product_id.parent IN ($it_parent_brand_id)";
		}

		if($it_child_brand_id != NULL and $it_child_brand_id != "-1"){
			$it_child_brand_id_condition= " AND it_terms_product_id.term_id IN ($it_child_brand_id)";
		}

		if($it_list_parent_brand != NULL and $it_list_parent_brand > 0){
			$it_list_parent_brand_condition= " AND it_term_taxonomy_product_id.parent > 0";
		}
		...
		if($brand_id  && $brand_id != "-1") {
			$brand_id_condition= " AND it_terms_product_id.term_id IN ($brand_id)";
		}


		$sql_group_by='';

		if($it_group_by_parent_brand == 1){
		...
		$sql = "SELECT $sql_columns FROM $sql_joins $it_id_order_status_join WHERE $sql_condition
				$it_id_order_status_condition $it_parent_brand_id_condition $it_child_brand_id_condition
				$it_list_parent_brand_condition $it_from_date_condition $it_publish_order_condition
				$it_order_status_condition $it_hide_os_condition $brand_id_condition
	...
	?>
		<form class='alldetails search_form_report' action='' method='post'>
            ...
            <div class="row">
                ...
                <?php
					$col_style='';
					$permission_value=$this->get_form_element_value_permission('it_parent_brand_id');
                ...
                <div class="col-md-6"  style=" <?php echo $col_style;?>">
                    <div class="awr-form-title">
                        <?php _e('Parent Brand',__IT_REPORT_WCREPORT_TEXTDOMAIN__);?>
                    </div>
					<span class="awr-form-icon"><i class="fa fa-tags"></i></span>
					<?php
                        $p_brands = $this->it_get_woo_sppc_data(__IT_BRAND_SLUG__);
                        $option='';
                    ...
                    ?>
                    <select name="it_parent_brand_id[]" multiple="multiple" size="5"  data-size="5" class="chosen-select-search">
            ...
            <div class="col-md-12 awr-save-form">
                    ...
                    <input type="hidden" name="it_brand_id" value="-1">
