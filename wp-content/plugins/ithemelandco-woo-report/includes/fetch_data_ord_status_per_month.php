<?php
	...
	if($file_used=="sql_table")
	{
		...
		$sql_condition = "shop_order.post_type	= 'shop_order' AND	it_postmeta2.meta_key 		= '_order_total'";
		...
		$sql = "SELECT $sql_columns FROM $sql_joins WHERE $sql_condition
			$it_id_order_status_condition $it_from_date_condition
		...
		//for($i=1; $i<=20 ; $i++){
			$datatable_value.=("<tr>");
				...
				//Status Name
				$it_table_value = isset($items->status_name) ? $items->status_name : '';
				...
				$items_product=$this->it_get_woo_items_status($type , $items_only, $id,$params);
