<?php
	if($file_used=="sql_table")
	{
		...
		$types = $wpdb->get_results("SELECT it_postmeta.meta_key as meta_key FROM ".$wpdb->postmeta." as it_postmeta INNER JOIN ".$wpdb->posts." as it_post ON it_postmeta.post_id=it_post.ID where it_post.post_type='product'  GROUP BY it_postmeta.meta_key", ARRAY_A);
		...
		$types = $wpdb->get_results("SELECT it_postmeta.meta_key as meta_key FROM ".$wpdb->postmeta." as it_postmeta INNER JOIN ".$wpdb->posts." as it_post ON it_postmeta.post_id=it_post.ID where it_post.post_type='shop_order' GROUP BY it_postmeta.meta_key", ARRAY_A);
			...
			foreach($custom_fields as $fields){
				...
				if($show_column=="on"){
					if(in_array($fields,$order_custom_fields))
						$order_custom_fields_cols[]=array('lable'=>__($label,__IT_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
					else
						$custom_fields_cols[]=array('lable'=>__($label,__IT_REPORT_WCREPORT_TEXTDOMAIN__),'status'=>'show');
				}
		...
		$sql .="$category_id_join $brand_id_join $all_tax_joins $it_id_order_status_join $it_txt_email_join $it_txt_first_name_join $all_fields_joins
		...
		$sql .= " Where $post_type_condition  $all_fields_conditions $it_txt_email_condition_1 $it_txt_first_name_condition_1
						...
						$it_product_id_condition $category_id_condition $brand_id_condition $all_tax_conditions  $it_id_order_status_condition
					...
					foreach($custom_fields as $fields){
						echo '
							<div class="col-md-6">
								...
								<span class="awr-form-icon"><i class="fa fa-map-marker"></i></span>
								<input name="it_'.$fields.'" type="text" placeholder="'.get_option($fields."_translate").'"/>
							</div>';
					}
				?>
                </div>
