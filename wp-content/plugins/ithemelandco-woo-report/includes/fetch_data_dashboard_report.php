...
//echo $today_refund;
  ...
  "July", "August", "September", "October", "November", "December");
		...
		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'red-1', 'fa-usd', esc_html__('Total Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_sales, 'price', $total_orders, 'number');

		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'blue-1', 'fa-reply-all', esc_html__('Total Refund',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_refund_amount, 'price', $total_refund_count, 'number');

		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'blue-2', 'fa-percent', esc_html__('Total Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_tax_amount, 'price', $total_tax_count, 'precent');

		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'brown-1', 'fa-ticket', esc_html__('Total Coupons',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_coupon_amount, 'price', $total_coupon_count, 'number');

		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'red-2', 'fa-user-plus', esc_html__('Total Registered',__IT_REPORT_WCREPORT_TEXTDOMAIN__), "#".$total_customer, 'other', '', 'number');

		$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'green-1', 'fa-user-o', esc_html__('Total Guest Customers',__IT_REPORT_WCREPORT_TEXTDOMAIN__), "#".$total_guest_customer, 'other', '', 'number');

		////ADDE IN VER4.0
		/// COST OF GOOD
		if(__IT_COG__!='') {
		    $total_summary .= $this->it_get_dashboard_boxes_generator($type, 'orange-2', 'fa-money', esc_html__('Cost of Good',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_cog, 'price', '', 'number');

			$total_summary .= $this->it_get_dashboard_boxes_generator($type, 'brown-2', 'fa-diamond', esc_html__('Total Profit',__IT_REPORT_WCREPORT_TEXTDOMAIN__), ($total_sales-$total_cog), 'price', '', 'number');
		}



		//echo '<div class="clearboth"></div><div class="awr-box-title">'.esc_html__('Other Summary',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div><div class="clearboth"></div>';

		$other_summary='';

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'orange-2', 'fa-area-chart', esc_html__('Cur. Yr Proj. Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'('.$it_cur_projected_sales_year.')', $total_projected_amount, 'price', $projected_percentage, 'precent');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'blue-2', 'piechart', esc_html__('Current Year Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'('.$it_cur_projected_sales_year.')', $projected_order_amount, 'price', $projected_order_count, 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'green-1', 'fa-line-chart', esc_html__('Average Sales Per Order',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_sales_avg, 'price', $total_orders, 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'pink-2', 'filter', esc_html__('Average Sales Per Day',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_sales_avg_per_day, 'price', $total_orders, 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'pink-2', 'like', esc_html__('Current Month Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'('.$projected_sales_month_shrt.' '.$it_cur_projected_sales_year.')', $projected_order_amount_cm, 'price', $projected_percentage_cm, 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'blue-2', 'fa-bar-chart', esc_html__('Cur. Month Proj. Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'('.$projected_sales_month_shrt.' '.$it_cur_projected_sales_year.')', $total_projected_amount_cm, 'price', $projected_order_count_cm, 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'blue-2', 'fa-pie-chart', '('.$projected_sales_month_shrt.' '.$it_cur_projected_sales_year.')'. esc_html__('Average Sales/Day',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $current_total_sales_apd, 'price', '', 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'brown-2', 'basket', '('.$projected_sales_month_shrt.' '.$it_cur_projected_sales_year.')'. esc_html__('Forecasted Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $sales_forcasted, 'price', '', 'number');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'orange-1', 'fa-percent', esc_html__('Order Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_ord_tax_amount, 'price', $total_ord_tax_count, 'precent');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'green-2', 'fa-truck', esc_html__('Order Shipping Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_ordshp_tax_amount, 'price', $total_ordshp_tax_count, 'precent');

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'red-2', 'filter', esc_html__('Order Shipping Total',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_orders_shipping, 'price', $total_orders, 'number');

		$amount='';
		$count='';
		if ( $last_order_date) $amount= date($short_date_format,$last_order_time); 	  else $amount=esc_html__( '0', __IT_REPORT_WCREPORT_TEXTDOMAIN__);

	   if ( $last_order_time_diff) $count= $last_order_time_diff; else $count=esc_html__( '0', __IT_REPORT_WCREPORT_TEXTDOMAIN__);

		$other_summary .= $this->it_get_dashboard_boxes_generator($type, 'green-2', 'fa-calendar', esc_html__('Last Order Date',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $amount, 'other', $count, 'other');


		//echo '<div class="clearboth"></div><div class="awr-box-title">'.esc_html__('Todays Summary',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</div><div class="clearboth"></div>';
		$today_summary='';
		$type='progress';

		$progress_html= $this->it_get_dashboard_progress_contents($total_today_sales,$total_yesterday_sales);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'red-1', 'fa-usd', esc_html__('Todays Total Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_today_sales, 'price', $total_today_order, 'number',$progress_html);

		$progress_html= $this->it_get_dashboard_progress_contents($total_today_avg,$total_yesterday_avg);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'orange-1', 'chart', esc_html__('Todays Average Sales',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $total_today_avg, 'price','', 'number',$progress_html);


		$progress_html= $this->it_get_dashboard_progress_contents($todays_refund_amount,$yesterday_refund_amount);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'pink-1', 'fa-reply-all', esc_html__('Todays Total Refund',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $todays_refund_amount, 'price', $todays_refund_count, 'number',$progress_html);


		$progress_html= $this->it_get_dashboard_progress_contents($today_coupon_amount,$yesterday_coupon_amount);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'brown-1', 'fa-ticket', esc_html__('Todays Total Coupons',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $today_coupon_amount, 'price', $today_coupon_count, 'number',$progress_html);

		$progress_html= $this->it_get_dashboard_progress_contents($today_tax_amount,$ytday_tax_amount);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'blue-2', 'fa-percent', esc_html__('Todays Order Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $today_ord_tax_amount, 'price', $today_ord_tax_count, 'number',$progress_html);

		$progress_html= $this->it_get_dashboard_progress_contents($today_tax_amount,$ytday_ordshp_tax_amount);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'red-2', 'fa-truck', esc_html__('Todays Shipping Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $today_ordshp_tax_amount, 'price', $today_ordshp_tax_count, 'number',$progress_html);

		$progress_html= $this->it_get_dashboard_progress_contents($today_tax_amount,$ytday_total_tax_amount);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'green-2', 'category', esc_html__('Todays Total Tax',__IT_REPORT_WCREPORT_TEXTDOMAIN__), $today_tax_amount, 'price', $today_tax_count, 'number',$progress_html);

		$progress_html= $this->it_get_dashboard_progress_contents($today_reg_customer,$yesterday_reg_customer);

		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'pink-2', 'fa-user-plus', esc_html__('Todays Registered Customers',__IT_REPORT_WCREPORT_TEXTDOMAIN__), "#".$today_reg_customer, 'other', '', 'number',$progress_html);


		$progress_html= $this->it_get_dashboard_progress_contents($today_guest_customer,$yesterday_guest_customer);
		$today_summary.= $this->it_get_dashboard_boxes_generator($type, 'orange-2', 'fa-user-o', esc_html__('Todays Guest Customers',__IT_REPORT_WCREPORT_TEXTDOMAIN__),  "#".$today_guest_customer, 'other', '', 'number',$progress_html);
			...
			<nav>
				<ul class="tab-uls">';
					...
					if($this->get_dashboard_capability('total_summary')){
						$htmls.='
					<li><a href="#section-bar-1" > <span>'.esc_html__('Total Summary',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</span></a></li>';
					}

					if($this->get_dashboard_capability('other_summary_box')){
						$htmls.='
					<li><a href="#section-bar-2" > <span>'.esc_html__('Other summary',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</span></a></li>';
					}

					if($this->get_dashboard_capability('today_summary')){
						$htmls.='
					<li><a href="#section-bar-3" > <span>'.esc_html__('Today summary',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</span></a></li>';
					}
					$htmls.='
				</ul>
			</nav>
	...
	?>
		<form class='alldetails search_form_reports' action='' method='post' id="dashboard_form">
			...
			<div class="page-toolbar">
				...
				<input type="hidden" name="publish_order" id="publish_order" value="<?php echo $it_publish_order;?>" />
