...
//		$datatable_value.=("</td>");
		...
		$blogusers = get_users( array( 'fields' => array( 'ID','display_name','user_email' ),'role'=>'customer' ) );
//print_r($blogusers); die;
		foreach ( $blogusers as $user ) {
//print_r($user->user_email);die;
			$type = 'total_row';$items_only = true; $id = $items->product_id;
			$it_from_date		  = $this->it_get_woo_requests('it_from_date',NULL,true);
			$it_to_date			= $this->it_get_woo_requests('it_to_date',NULL,true);
			$it_order_status		= $this->it_get_woo_requests('it_orders_status','-1',true);
			$it_order_status  		= "'".str_replace(",","','",$it_order_status)."'";
			$it_from_date=substr($it_from_date,0,strlen($it_from_date)-3)."-01";
			$it_to_date=substr($it_to_date,0,strlen($it_to_date)-3).'-31';
			...
			$params=array(
				...
				"it_hide_os"=>'"trash"',
				"users"=>array($user->ID,$user->user_email),

			);

			//echo $user->user_email.'-';

			$product_per_users= $this->it_get_product_purchase_per_user($type,$items_only,$id,$params);
//die(print_r($product_per_users));
