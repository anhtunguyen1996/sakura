<?php
	...
	if($file_used=="sql_table")
	{
		...
		$it_payment_gatway		= $this->it_get_woo_requests('it_pay_gatway','-1',true);

		if($it_payment_gatway != NULL  && $it_payment_gatway != '-1')
		{
			$it_payment_gatway = str_replace(",", "','",$it_payment_gatway);
		}
		...
		$sql_columns = "
		it_postmeta1.meta_value 							as ids
		...
		if(isset($it_payment_gatway) && $it_payment_gatway != NULL  && $it_payment_gatway != '-1')
			$it_payment_gatway_condition = "
				AND	it_postmeta1.meta_value IN ('{$it_payment_gatway}')";
		...
		$sql = "SELECT $sql_columns FROM $sql_joins $it_id_order_status_joins WHERE $sql_condition
			$it_id_order_status_condition $it_from_date_condition $it_payment_gatway_condition
		...
		//for($i=1; $i<=20 ; $i++){
			$datatable_value.=("<tr>");
				...
				$items_product=$this->it_get_woo_pg_items($type , $items_only, $id,$params);
		...
		<form class='alldetails search_form_report' action='' method='post'>
			...
			<div class="row">
                ...
                <div class="col-md-6">
                	<div class="awr-form-title">
						<?php _e('Payment Gatway',__IT_REPORT_WCREPORT_TEXTDOMAIN__);?>
                    </div>
					<span class="awr-form-icon"><i class="fa fa-credit-card"></i></span>
					<?php

                        $gateway_data = $this->it_get_woo_pay_gateways();
                        $option='';
                    ...
                    <select name="it_pay_gatway[]" multiple="multiple" size="5"  data-size="5" class="chosen-select-search">
