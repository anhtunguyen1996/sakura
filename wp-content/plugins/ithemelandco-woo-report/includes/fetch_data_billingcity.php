<?php
	...
	if($file_used=="sql_table")
	{
		...
		,COALESCE((select meta_value from {$wpdb->prefix}postmeta  where meta_key='_billing_city' and post_id=it_posts.ID),'') AS 'billing_city_code'
		,COALESCE((select meta_value from {$wpdb->prefix}postmeta  where meta_key='_billing_state' and post_id=it_posts.ID),'') AS 'billing_state_code'
		,COALESCE((select meta_value from {$wpdb->prefix}postmeta  where meta_key='_billing_country' and post_id=it_posts.ID),'default') AS 'billing_country'
	...
	";
		...
		//for($i=1; $i<=20 ; $i++){
			...
			$datatable_value.=("<tr>");
				...
				if($this->table_cols[$index_cols++]['status']=='hide') $display_class='display:none';
				$datatable_value.=("<td style='".$display_class."'>");
					$datatable_value.= $this->it_get_woo_bsn($items->billing_country,$items->billing_state_code);
				$datatable_value.=("</td>");
