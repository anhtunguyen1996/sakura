...
class ITWR_WP_Register_Update
{
    ...
    public function check_update($transient)
    {
        ...
        if (!empty($validation) && $validation == 'valid') {
            if (!empty($update_data) && is_array($update_data)) {
                ...
                if ($remote_version) {
                    $plugin_basename = __IT_REPORT_WCREPORT_TEXTDOMAIN__ . '/' . __IT_REPORT_WCREPORT_TEXTDOMAIN__ . '.php';
    ...
    public function set_plugins_api($obj, $action, $arg)
    {
        ...
        if (!empty($update_data) && is_array($update_data)) {
            if (($action == 'query_plugins' || $action == 'plugin_information') && isset($arg->slug) && $arg->slug === __IT_REPORT_WCREPORT_TEXTDOMAIN__) {
    ...
    private function get_plugin_update_data($action = null, $update_data = [])
    {
        ...
        if ($action == 'info') {
            ...
            $data->last_updated = date('Y-m-d', $update_data['date']);
