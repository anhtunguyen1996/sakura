<?php
	...
	if(!function_exists('add_custom_meta_advanced_reporting')){
		...
		function add_custom_meta_advanced_reporting() {
			...
			add_meta_box(
				'ithemelandco_main_page', // $id
				'<i class="fa fa-th-list"></i> '.esc_html__('Cost Of Good',IT_PRO_BOX_LAYOUT_TEXTDOMAIN), // $title
	...
	///////////////////END METABOXES//////////////////////////////////
	global $it_customfiled_query;
	$it_customfiled_query= array(

		array(
			'label' => '<strong>'.esc_html__('View',IT_PRO_BOX_LAYOUT_TEXTDOMAIN).'</strong>',
			'desc'  => esc_html__('Set count of view',IT_PRO_BOX_LAYOUT_TEXTDOMAIN),
			'id'    => 'pl_view_post',
			'type'  => 'numeric',
		),
		array(
			'label' => '<strong>'.esc_html__('Like',IT_PRO_BOX_LAYOUT_TEXTDOMAIN).'</strong>',
			'desc'  => esc_html__('Set count of like',IT_PRO_BOX_LAYOUT_TEXTDOMAIN),
			'id'    => 'pl_like_post',
			'type'  => 'numeric',
		),
		array(
			'label' => '<strong>'.esc_html__('Video Url',IT_PRO_BOX_LAYOUT_TEXTDOMAIN).'</strong>',
			'desc'  => esc_html__('Set embed video url',IT_PRO_BOX_LAYOUT_TEXTDOMAIN),
			'id'    => 'pl_video_url',
			'type'  => 'textbox',
		),
	);
	...
	function show_customfields() {
		global $it_customfiled_query, $post;
		...
		echo '<input type="hidden" name="show_custom_meta_box_extra_bulkedit_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';
			...
			foreach ($it_customfiled_query as $field) {


				$meta = get_post_meta($post->ID, $field['id'], true);
				if($field['type']=='hidden')
				{
					echo '<input type="text" name="'. $field['id'].'" class="it_step_order" value="'.$meta.'"/>';
					continue;
				}
