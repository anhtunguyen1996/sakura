<?php
	...
	$it_active_plugin = array(
		...
		array(
			'label'	=> esc_html__('Purchase Code',__IT_REPORT_WCREPORT_TEXTDOMAIN__),
			'desc'	=> esc_html__('Enter Your Purchase Code',__IT_REPORT_WCREPORT_TEXTDOMAIN__),
		...
		array(
			'label'	=> esc_html__('Email',__IT_REPORT_WCREPORT_TEXTDOMAIN__),
			'desc'	=> esc_html__('Enter Your Valid Email.',__IT_REPORT_WCREPORT_TEXTDOMAIN__),
	...
	if (isset($_POST["update_settings"])) {
		...
		if ($it_rpt_main_class->dashboard($it_rpt_main_class->it_plugin_status) && isset($check_db["verify-purchase"]["status"]) && $check_db["verify-purchase"]["status"]=='valid' && filter_var($it_rpt_main_class->email, FILTER_VALIDATE_EMAIL)){
			$text=esc_html__('Congratulation, The Plugin has been Activated Successfully !',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
			?>
                <script>
			...
			$text=esc_html__('Unfortunately, The Purchase code is Wrong, Please try Again !',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
    ...
    $text_ok=esc_html__('Congratulation, The Plugin has been Activated Successfully ! Move to ',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'<a href="admin.php?page='.$it_rpt_main_class->it_plugin_main_url.'">'.esc_html__("Dashboard",__IT_REPORT_WCREPORT_TEXTDOMAIN__).'</a>';
    $text_error=esc_html__('Unfortunately, The Purchase code is Wrong or Email is not Valid, Please try Again !',__IT_REPORT_WCREPORT_TEXTDOMAIN__);
    ...
    <div class="wrap">
        <div class="row">
                <div class="col-xs-12">
                    <div class="awr-box">
                            <div class="awr-title">
                                <h3><i class="fa fa-shield"></i>'.esc_html__('Plugin Activate',__IT_REPORT_WCREPORT_TEXTDOMAIN__).'  </h3>
                            </div><!--awr-title -->
                                ...
                                <div class="col-md-12">
                                    ...
                                    if(!filter_var($it_rpt_main_class->email, FILTER_VALIDATE_EMAIL)) {
                                        ...
                                        <div id="setting-error-settings_updated" class="updated email-notice it_active_email">
                                            <p><strong>' . esc_html__( 'Please set email for complete activation in Ver4.0', __IT_REPORT_WCREPORT_TEXTDOMAIN__ ) . '</strong></p>
                                        </div>';
                                    }
                                    $html.= '
                                </div>
